...
StudentFitNULL2 <- brm(
  bf(
    # VMAX ~ offset(HWRF), family = gaussian()
    # VMAX ~ HWRF, family = gaussian(link = "log")
    VMAX ~ log(HWRF), family = student(link = "log")
    # VMAX ~ log(HWRF), family = gaussian(link = "log)
  ),
  data = StormdataTrain8, 
  save_pars = save_pars(all = TRUE), 
...
print(StudentFitNULL, digits = 4)
plot(StudentFitNULL)
StudentFitNULLppcFit <- pp_check(StudentFitNULL, ndraws = 100) + 
...
##### Prediction ----
## Fitted
StudentFitNULLfinalFit <- posterior_predict(StudentFitNULL)
...
## Prediction on new data
StudentFitNULLfinalPreds <- posterior_predict(StudentFitNULL, 
                                          newdata = StormdataTest8,
                                          allow_new_levels = TRUE)
...
gammaFitNULL2 <- brm(
  bf(
    VMAX ~ log(HWRF), family = Gamma(link = "log")
    # VMAX ~ log(HWRF), family = lognormal(),
    # VMAX ~ log(HWRF), family = lognormal(),
    # VMAX ~ log(HWRF), family = lognormal(),
  ),
...
## Prediction
gammaFitNULL2PredDF <- bind_cols(
  StormdataTest3,
  LCB = gammaFitNULL2finalPredsLCB,
  Mean = gammaFitNULL2finalPredsMean,
  Med = gammaFitNULL2finalPredsMed,
  UCB = gammaFitNULL2finalPredsUCB
) |>
  mutate(
    VMAX = Actual_Yvec
  ) #|>
...
gammaFitNULL2stormsPredplot <- ggplot(data = gammaFitNULL2PredDF, aes(x = StormElapsedTime)) +
  geom_ribbon(aes(ymin = LCB, ymax = UCB), fill = "lightblue") +
  geom_line(aes(y = VMAX, color = "Observed")) +
  geom_line(aes(y = Mean, color = "PPD Mean")) +
  facet_wrap(vars(StormID), ncol = 6)+
...
predCompNULLnoOffset <- predCompNULL |> arrange(MAE_pred)
